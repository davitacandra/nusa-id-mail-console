...
export const addCompany = async (req: Request, res: Response): Promise<Response> => {
    const { companyName, companyAddress, maxDomain, maxMailAccount, maxMailQuota } = req.body as CompanyData
    ...
    try {
        ...
        const [insertResult] = await connection.promise().execute<OkPacket>(insertQuery, [companyName, companyAddress, maxDomain, maxMailAccount, maxMailQuota])
...
export const deleteCompany = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        const [deleteResult] = await connection.promise().execute<OkPacket>(deleteQuery, [companyId])
