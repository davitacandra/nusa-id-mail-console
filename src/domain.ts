...
export const addDomain = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        const [domainCountResult] = await connection.promise().execute<RowDataPacket[]>(domainCountQuery, [companyId])
        ...
        const [maxDomainResult] = await connection.promise().execute<RowDataPacket[]>(maxDomainQuery, [companyId])
        ...
        const verificationString = `nusa.id-mail-verification=${randomBytes(20).toString('hex')}`
        ...
        const [insertResult] = await connection.promise().execute(insertQuery, [domainName, companyId, verificationString])
...
export const verifyDomain = async (req: Request, res: Response): Promise<Response> => {
    ...
    try {
        ...
        const [checkResult] = await connection.promise().query<RowDataPacket[]>(checkQuery, [domainId])
        ...
        if (loggedInAdminType === 'superadmin' || (loggedInAdminType === 'admin' && loggedInCompanyId === domainCompanyId) || (loggedInAdminType === 'operator' && loggedInCompanyId === domainCompanyId)) {
            ...
            const [updateResult] = await connection.promise().execute<OkPacket>(updateQuery, [domainId])
...
export const deleteDomain = async (req: Request, res: Response): Promise<Response> => {
    const { domainId } = req.params
    const loggedInAdminType = req.adminType
    const loggedInCompanyId = req.companyId
    ...
    try {
        ...
        if (loggedInAdminType === 'superadmin' || (loggedInAdminType === 'admin' && loggedInCompanyId === domainCompanyId) || (loggedInAdminType === 'operator' && loggedInCompanyId === domainCompanyId)) {
            ...
            const [deleteResult] = await connection.promise().execute<OkPacket>(deleteQuery, [domainId])
...
export const showDomain = async (req: Request, res: Response): Promise<Response> => {
    const loggedInAdminType = req.adminType
    const loggedInCompanyId = req.companyId
    ...
    try {
        ...
        const [domains] = await connection.promise().query<RowDataPacket[]>(query, queryParams)
