...
export const getProfile = async (req: Request, res: Response) => {
  ...
  try {
    ...
    const [adminRows] = await connection.promise().query<RowDataPacket[]>(adminQuery, [loggedInUsername])
    ...
    const [companyRows] = await connection.promise().query<RowDataPacket[]>(companyQuery, [admin.company_id])
...
export const changePassword = async (req: Request, res: Response) => {
  ...
  try {
      ...
      const [results] = await connection.promise().query<RowDataPacket[]>(query, [loggedInUsername])
      ...
      const passwordIsValid = await bcrypt.compare(currentPassword, user.admin_password)
      ...
      const [updateResults] = await connection.promise().execute<OkPacket>(updateQuery, [hashedNewPassword, loggedInUsername])
