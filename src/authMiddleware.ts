...
export const checkAdminType = (requiredTypes: string[]) => {
  return async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
      ...
      try {
          ...
          const [rows] = await connection.promise().query<RowDataPacket[]>(query, [loggedInUsername])
...
export const preventSelfModification = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
  const loggedInUsername = req.user?.username as string
...
export const attachUserInfo = async (req: Request, res: Response, next: NextFunction): Promise<Response | void> => {
  const loggedInUsername = req.user?.username as string
  const { adminId } = req.params
  ...
  try {
      ...
      const [userRows] = await connection.promise().query<RowDataPacket[]>(userQuery, [loggedInUsername])
      ...
      if (adminId) {
          ...
          const [targetUserRows] = await connection.promise().query<RowDataPacket[]>(targetUserQuery, [adminId])
