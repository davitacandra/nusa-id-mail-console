...
exports.manageAdmin = exports.showAdmin = exports.deleteAdmin = exports.addAdmin = void 0;
const db_config_1 = __importDefault(require("./db-config"));
const bcrypt_1 = __importDefault(require("bcrypt"));
const addAdmin = async (req, res) => {
    const { fullname, username, password, type } = req.body;
    let { companyId } = req.body; // This can be overridden for 'admin' users
    const loggedInAdminType = req.adminType; // Provided by attachUserInfo middleware
    const loggedInCompanyId = req.companyId; // Provided by attachUserInfo middleware
    const allowedTypesForAdmin = ['admin', 'guest', 'operator'];
    if (!fullname || !username || !password || !type) {
        return res.status(400).json({ message: 'Full name, username, password, and type are required' });
    }
    ...
    if (loggedInAdminType === 'admin') {
        ...
        if (!allowedTypesForAdmin.includes(type)) {
            return res.status(403).json({ message: 'Unauthorized - Admin can only set types to ${allowedTypesForAdmin.join(`, `)}' });
        }
    }
    ...
    try {
        ...
        const [insertResult] = await db_config_1.default.promise().execute(insertQuery, [username, hashedPassword, fullname, type, companyId
        ]);
        if (insertResult) {
            return res.status(201).json({ message: 'Account added successfully', admin: username });
        }
        else {
...
const deleteAdmin = async (req, res) => {
    ...
    if (loggedInAdminType === 'admin' &&
        (!allowedTypesForAdmin.includes(targetAdmin.type) || targetAdmin.companyId !== loggedInCompanyId)) {
        return res.status(403).json({ message: 'Unauthorized - Can only delete admin, operator, and guest within the same company' });
    }
    try {
        const deleteQuery = `DELETE from mailgw_admin WHERE admin_id = ?`;
        const [deleteResult] = await db_config_1.default.promise().execute(deleteQuery, [adminId]);
...
const showAdmin = async (req, res) => {
    ...
    try {
        const [admins] = await db_config_1.default.promise().query(query, queryParams);
...
const manageAdmin = async (req, res) => {
    ...
    const targetAdmin = req.targetAdmin; // Added from the middleware
    // Define the types that an 'admin' can set
    const allowedTypesForAdmin = ['admin', 'guest', 'operator'];
    if (!adminId || !newType) {
        return res.status(400).json({ message: 'Admin ID and new type are required' });
    }
    ...
    if (adminType === 'admin' && targetAdmin?.type === 'superadmin') {
        return res.status(403).json({ message: 'Unauthorized - Cannot change the type of a superadmin' });
    }
    ...
    if (adminType === 'admin' && targetAdmin?.companyId !== companyId) {
        return res.status(403).json({ message: 'Unauthorized - Can only update admin within the same company' });
    }
    try {
        ...
        if (newPassword) {
            const hashedPassword = await bcrypt_1.default.hash(newPassword, 10); // Hash the new password
            if (newType) {
                updateQuery += `, `;
            }
        ...
        const [updateResult] = await db_config_1.default.promise().execute(updateQuery, updateParams);
